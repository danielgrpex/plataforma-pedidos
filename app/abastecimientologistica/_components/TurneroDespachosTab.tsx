...
export function TurneroDespachosTab() {
  ...
  function limpiarTextoExcel(value: any) {
    return String(value ?? "")
      .replace(/\r?\n|\r/g, " ")
      .replace(/\s+/g, " ")
      .trim();
  ...
  function celdaCsv(value: any) {
    const limpio = limpiarTextoExcel(value);
    return `"${limpio.replace(/"/g, '""')}"`;
  }

  function exportarProgramacionExcel() {
    const registros = [...represados, ...items];

    if (!registros.length) {
      setMessage("No hay programación visible para exportar.");
      return;
    }

    const headers = [
      "PRIORIDAD",
      "CLIENTE - TIENDA/DIRECCIÓN",
      "FECHA PROGRAMADA",
    ];

    const rows = registros.map((r, idx) => {
      const cliente = limpiarTextoExcel(r.cliente || r.pedidosKey || "-");
      const direccion = limpiarTextoExcel(r.direccion || "-");
      const oc = limpiarTextoExcel(r.oc || "-");
      const fechaProgramada = limpiarTextoExcel(r.fechaEstimadaDespacho || fecha);

      return [
        idx + 1,
        `${cliente} - Dirección: ${direccion} · OC: ${oc}`,
        fechaProgramada,
      ];
    });

    const csv = [headers, ...rows]
      .map((row) => row.map(celdaCsv).join(";"))
      .join("\r\n");
    ...
    a.href = url;
    a.download = `programacion-despachos-${fecha}.csv`;
    ...
    setMessage(`Programación exportada correctamente. Registros: ${registros.length}`);
  }
  ...
  const totalExportable = represados.length + items.length;
    ...
    <section className="rounded-2xl border border-neutral-200 bg-white p-5">
      ...
      <div className="mt-5 flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            onClick={exportarProgramacionExcel}
            disabled={loading || totalExportable === 0}
            className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700 shadow-sm hover:bg-emerald-100 disabled:opacity-60"
          >
            Exportar Excel
          </button>
