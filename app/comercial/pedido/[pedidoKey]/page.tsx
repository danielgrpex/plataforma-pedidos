...
export default function VerPedidoPage() {
  ...
  const totalItems = useMemo(() => pedido?.items?.length ?? 0, [pedido]);
    const soporteEntrega = useMemo(() => {
    if (!pedido) return null;

    if (pedido.soporteEntregaUrl) {
      return {
        url: pedido.soporteEntregaUrl,
        nombre: pedido.soporteEntregaNombre || "Documento soporte de entrega",
        fechaCargue: pedido.fechaCargueSoporteEntrega,
        usuario: pedido.usuarioEntregaCliente,
        observaciones: pedido.observacionesEntregaCliente,
      };
    }

    const itemConSoporte = pedido.items.find((it) => it.soporteEntregaUrl);

    if (!itemConSoporte?.soporteEntregaUrl) return null;

    return {
      url: itemConSoporte.soporteEntregaUrl,
      nombre: itemConSoporte.soporteEntregaNombre || "Documento soporte de entrega",
      fechaCargue: itemConSoporte.fechaCargueSoporteEntrega,
      usuario: itemConSoporte.usuarioEntregaCliente,
      observaciones: itemConSoporte.observacionesEntregaCliente,
    };
  }, [pedido]);
                  ...
                  <div>
                    ...
                    {soporteEntrega ? (
                      <div className="mt-2 space-y-1 text-sm text-slate-600">
                        <p>
                          <span className="font-medium text-slate-800">Archivo:</span>{" "}
                          {soporteEntrega.nombre}
                        </p>

                        {soporteEntrega.fechaCargue ? (
                          <p>
                            <span className="font-medium text-slate-800">Fecha cargue:</span>{" "}
                            {formatFechaColombia(soporteEntrega.fechaCargue)}
                          </p>
                        ) : null}

                        {soporteEntrega.usuario ? (
                          <p>
                            <span className="font-medium text-slate-800">Cargado por:</span>{" "}
                            {soporteEntrega.usuario}
                          </p>
                        ) : null}
